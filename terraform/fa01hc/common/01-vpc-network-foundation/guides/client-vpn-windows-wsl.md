...
| `nc`는 성공하지만 SSH가 `expecting SSH2_MSG_KEX_ECDH_REPLY`에서 멈춤 | `client-config-mtu.ovpn`에 `tun-mtu 1400`, `mssfix 1360`을 추가해 다시 연결합니다. 계속 멈추면 `tun-mtu 1300`, `mssfix 1260`으로 낮춥니다. |
